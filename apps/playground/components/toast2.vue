<template>
  <RootPortal v-if="toasts">
    <View :class="rootClasses" :style="'font-family:' + fontFamily">
      <View class="kd-toast-area" :style="areaStyle">
        <View class="kd-toast-wrapper" v-for="toast in innerToasts" :key="toast.id">
          <View :class="toast.classes" @transitionend="onTransitionEnd($event, toast)">
...
import { computed, ref, onAttached } from '@minivue/core'
import {
  fontFamily,
  classObjectToString,
  getAppBaseInfo,
...
const areaStyle = computed(() => `margin-top: ${navbarHeight.value}px`)
const rootClasses = computed(() => `kd-toast-root kd-theme--default kd-theme--${theme.value}`)
const toasts = ref<Toast[]>([])

const innerToasts = computed(() =>
  toasts.value.map((item) => {
    item.iconSize = item.hud ? 48 : 22
    item.isAutoHide =
      item.duration !== undefined &&
      item.duration > 0 &&
      !['progress', 'loading'].includes(item.icon as string)
    item.classes = classObjectToString('kd-toast', {
      'kd-toast--hud': item.hud,
      'kd-toast--hudtext': item.hud && item.content,
      'kd-toast--full': item.action,
      'kd-toast--show': item.show,
    })
    return item
  }),
)
...
page.$showToast = (options: KdToastOptions<boolean>) => {
  ...
  options.show = true
  const isAutoHide =
    options.duration > 0 && !['progress', 'loading'].includes(options.icon as string)
  if (isAutoHide) {
